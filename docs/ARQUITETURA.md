...
- **Exclusão protegida**: `DELETE /clientes/:id` retorna `409` se houver qualquer compra ou crediário vinculado, mesmo sem dívida pendente. Como novos clientes recebem crediário automaticamente, a exclusão `204` fica disponível para cadastros antigos sem esses vínculos. Quando permitida, a exclusão remove os endereços em cascata.
